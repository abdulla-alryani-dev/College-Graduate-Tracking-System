    ...
    <div class="form-container p-4">
        <header class="mb-5 text-center">
            <h1 class="h2 text-primary fw-bold">
                <i class="fas fa-file-contract me-2"></i>نموذج تعديل عرض وظيفي
            </h1>
            <p class="text-muted">قم بتعديل الحقول المطلوبة ثم احفظ التغييرات</p>
        </header>

        <form id="jobForm" action="{{route('offers.update', $offer->id)}}" novalidate method="POST">
            @method('POST')
            @csrf
            <!-- القسم الأساسي -->
            <div class="dynamic-section">
                ...
                <div class="row g-4">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control form-control-lg" id="job_title" name="job_title" value="{{ $offer->job_title }}" required>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select form-select-lg" id="job_category" name="job_category" required>
                            ...
                            <option value="تقنية المعلومات" {{ $offer->job_category == 'تقنية المعلومات' ? 'selected' : '' }}>تقنية المعلومات</option>
                            <option value="التسويق الرقمي" {{ $offer->job_category == 'التسويق الرقمي' ? 'selected' : '' }}>التسويق الرقمي</option>
                            <option value="إدارة الأعمال" {{ $offer->job_category == 'إدارة الأعمال' ? 'selected' : '' }}>إدارة الأعمال</option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select form-select-lg" id="job_type" name="job_type" required>
                            ...
                            <option value="دوام كامل" {{ $offer->job_type == 'دوام كامل' ? 'selected' : '' }}>دوام كامل</option>
                            <option value="دوام جزئي" {{ $offer->job_type == 'دوام جزئي' ? 'selected' : '' }}>دوام جزئي</option>
                            <option value="عقد مؤقت" {{ $offer->job_type == 'عقد مؤقت' ? 'selected' : '' }}>عقد مؤقت</option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select form-select-lg" id="experience_level" name="experience_level" required>
                            ...
                            <option value="مبتدئ (0-2 سنة)" {{ $offer->experience_level == 'مبتدئ (0-2 سنة)' ? 'selected' : '' }}>مبتدئ (0-2 سنة)</option>
                            <option value="متوسط (2-5 سنوات)" {{ $offer->experience_level == 'متوسط (2-5 سنوات)' ? 'selected' : '' }}>متوسط (2-5 سنوات)</option>
                            <option value="خبير (5-10 سنوات)" {{ $offer->experience_level == 'خبير (5-10 سنوات)' ? 'selected' : '' }}>خبير (5-10 سنوات)</option>
                        </select>
                    </div>

                    <div class="col-12">
                        <label for="job_description" class="form-label fw-medium">وصف الوظيفة <span class="text-danger">*</span></label>
                        <textarea class="form-control form-control-lg" id="job_description" name="job_description" rows="5" required>{{ $offer->job_description }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="dynamic-section">
                ...
                <div class="row g-4">
                        ...
                        <div class="form-group">
                            ...
                            <div class="d-flex gap-4">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="location_type" id="remote" value="remote" {{ $offer->location_type == 'remote' ? 'checked' : '' }} required>
                                    <label class="form-check-label" for="remote">عن بُعد</label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="location_type" id="hybrid" value="hybrid" {{ $offer->location_type == 'hybrid' ? 'checked' : '' }}>
                                    <label class="form-check-label" for="hybrid">هجين</label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="location_type" id="onsite" value="onsite" {{ $offer->location_type == 'onsite' ? 'checked' : '' }}>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control form-control-lg" id="location" name="location" value="{{ $offer->location }}">
                    </div>

                    <div class="col-md-6">
                        <label for="vacancies" class="form-label fw-medium">عدد الشواغر <span class="text-danger">*</span></label>
                        <input type="number" class="form-control form-control-lg" id="vacancies" name="vacancies" min="1" value="{{ $offer->vacancies }}" required>
                    </div>

                    <div class="col-md-6">
                        <label for="expiration_date" class="form-label fw-medium">تاريخ الانتهاء <span class="text-danger">*</span></label>
                        <input type="date" class="form-control form-control-lg" id="expiration_date" name="expiration_date" value="{{ $offer->expiration_date }}" required>
                    </div>
                </div>
            </div>
                ...
                <div class="row g-4">
                    <div class="col-md-6">
                        ...
                        <div class="d-flex gap-4 mb-4">
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="salary_type" id="fixedSalary" value="fixed" {{ $offer->salary_type == 'fixed' ? 'checked' : '' }} required>
                                <label class="form-check-label" for="fixedSalary">راتب ثابت</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="salary_type" id="salaryRange" value="range" {{ $offer->salary_type == 'range' ? 'checked' : '' }}>
                                <label class="form-check-label" for="salaryRange">نطاق رواتب</label>
                            </div>
                        </div>

                        <div id="fixedSalaryFields" class="salary-input-group {{ $offer->salary_type == 'range' ? 'd-none' : '' }}">
                            <input type="number" class="form-control form-control-lg" id="fixed_salary" name="fixed_salary" placeholder="المبلغ" value="{{ $offer->fixed_salary }}">
                            <select class="form-select form-select-lg" id="fixed_salary_currency" name="fixed_salary_currency">
                                <option value="SAR" {{ $offer->fixed_salary_currency == 'SAR' ? 'selected' : '' }}>SAR</option>
                                <option value="USD" {{ $offer->fixed_salary_currency == 'USD' ? 'selected' : '' }}>USD</option>
                                <option value="EUR" {{ $offer->fixed_salary_currency == 'EUR' ? 'selected' : '' }}>EUR</option>
                            </select>
                            <select class="form-select form-select-lg" id="fixed_salary_period" name="fixed_salary_period">
                                <option value="شهري" {{ $offer->fixed_salary_period == 'شهري' ? 'selected' : '' }}>شهري</option>
                                <option value="سنوي" {{ $offer->fixed_salary_period == 'سنوي' ? 'selected' : '' }}>سنوي</option>
                            </select>
                        </div>

                        <div id="salaryRangeFields" class="salary-input-group {{ $offer->salary_type == 'fixed' ? 'd-none' : '' }}">
                            <input type="number" class="form-control form-control-lg" id="salary_min" name="salary_min" placeholder="الحد الأدنى" value="{{ $offer->salary_min }}">
                            <input type="number" class="form-control form-control-lg" id="salary_max" name="salary_max" placeholder="الحد الأقصى" value="{{ $offer->salary_max }}">
                            <select class="form-select form-select-lg" id="salary_range_currency" name="salary_range_currency">
                                <option value="SAR" {{ $offer->salary_range_currency == 'SAR' ? 'selected' : '' }}>SAR</option>
                                <option value="USD" {{ $offer->salary_range_currency == 'USD' ? 'selected' : '' }}>USD</option>
                                <option value="EUR" {{ $offer->salary_range_currency == 'EUR' ? 'selected' : '' }}>EUR</option>
                            </select>
                            <select class="form-select form-select-lg" id="salary_range_period" name="salary_range_period">
                                <option value="شهري" {{ $offer->salary_range_period == 'شهري' ? 'selected' : '' }}>شهري</option>
                                <option value="سنوي" {{ $offer->salary_range_period == 'سنوي' ? 'selected' : '' }}>سنوي</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        <div class="technical-tools-group">
                            ...
                            <div class="tag-container" id="toolsContainer">

                            </div>
                            <input type="hidden" id="technical_tools" name="technical_tools" value="{{ $offer->technical_tools }}">
                        </div>
                    </div>

                    <div class="col-12">
                        <label for="qualifications" class="form-label fw-medium">المؤهلات العلمية</label>
                        <textarea class="form-control form-control-lg" id="qualifications" name="qualifications" rows="3">{{ $offer->qualifications }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="dynamic-section">
                ...
                <div class="row g-4">
                    <div class="col-12">
                        ...
                        <textarea class="form-control form-control-lg" id="application_instructions" name="application_instructions" rows="3" required>{{ $offer->application_instructions }}</textarea>
                    </div>

                    <div class="col-12">
                        <label for="additional_info" class="form-label fw-medium">معلومات إضافية</label>
                        <textarea class="form-control form-control-lg" id="additional_info" name="additional_info" rows="3">{{ $offer->additional_info }}</textarea>
                    </div>

                    <div class="col-md-6">
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" id="is_active" name="is_active" {{ $offer->is_active ? 'checked' : '' }}>
        ...
        form.addEventListener('submit', (e) => {
            ...
            fetch(form.action, {  // Use the form's action URL
                ...
                headers: {
                    ...
                    'X-HTTP-Method-Override': 'POST'  // For Laravel method override
                },
                ...
                .then(data => {
                    ...
                    alert('تم تحديث العرض الوظيفي بنجاح!');
                })
                .catch(error => {
                    console.error('Error:', error);
                    alert('حدث خطأ أثناء تحديث العرض الوظيفي.');
                });
        });
